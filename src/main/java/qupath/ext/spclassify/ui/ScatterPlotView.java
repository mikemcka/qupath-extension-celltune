...
import qupath.ext.spclassify.model.LeidenModel;
import qupath.ext.spclassify.model.PopulationSet;
import qupath.ext.spclassify.model.ScatterMath;
...
public class ScatterPlotView {
    ...
    public ScatterPlotView(
        ...
        stage.setTitle("SP Classify — Cell Scatter Plot");
     ...
     * (PROJECT scope). The SP Classify ground-truth label store is never touched.
     */
    private void applyClustersToClasses() {
        ...
        if (counts == null) {
            Dialogs.showWarningNotification("SP Classify", "No clusters available yet — run Recompute first.");
            return;
        }
        if (scope == Scope.CURRENT_IMAGE && hierarchy == null) {
            Dialogs.showWarningNotification("SP Classify", "No image is open to classify.");
            return;
        }
        if (scope == Scope.PROJECT && fitCentroids == null) {
            Dialogs.showWarningNotification("SP Classify", "No fit available yet — run Recompute first.");
            return;
        }
    ...
    private void assignAcrossProject(Map<Integer, PathClass> mapping) {
        ...
        if (project == null) {
            Dialogs.showErrorMessage("SP Classify", "No project is open.");
            return;
        }
    ...
    private void assignAcrossProjectByMeasurement(Map<Integer, PathClass> mapping) {
        ...
        new Thread(
                        ...
                        "SpClassify-CohortAssignMeasurement")
                .start();
    }
    ...
    private void showClusterOverlay() {
        ...
        if (viewer == null || viewer.getImageData() == null) {
            Dialogs.showWarningNotification("SP Classify", "No open image to recolour.");
            return;
        }
    ...
    private void switchToImageScope() {
        ...
        if (hierarchy == null) {
            Dialogs.showErrorMessage("SP Classify", "No image is open.");
    ...
    private void switchToProjectScope() {
        ...
        if (project == null) {
            Dialogs.showErrorMessage("SP Classify", "Open a project to cluster across multiple images.");
            imageScopeToggle.setSelected(true);
            return;
        }
    ...
    private void exportAsPng() {
        ...
        try {
            ...
            Dialogs.showInfoNotification("SP Classify", "Exported scatter plot: " + file.getName());
            logger.info("Exported cell scatter plot to {}", file);
        } catch (IOException ex) {
            logger.error("Failed to export scatter plot", ex);
            Dialogs.showErrorMessage("SP Classify", "Failed to export PNG: " + ex.getMessage());
        }
    }
}
